    ...
    log(scItem)
		log(getContextPath())
		log('/shop/cart/addShoppingCartItem')
		$.ajax({
			 type: 'POST',
         ...
         log('add cart from webapp shop mini cart')
...
function displayMiniCartSummary(code){
	$.ajax({
		 ...
		 success: function(cart) {
       log('display minicart from webapp shopminicart')
			 if(cart==null || cart=='') {
					emptyCartLabel();
					$.cookie('cart',null, { expires: 1, path:'/' });
			 } else {
