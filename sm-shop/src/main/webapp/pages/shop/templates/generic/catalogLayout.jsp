...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
 ...
 <html xmlns="http://www.w3.org/1999/xhtml">
			...
			{{#products}}
                        <div itemscope itemtype="http://schema.org/Enumeration" id = "{{description.id}}" class="col-md-COLUMN-SIZE col-sm-6 col-xs-12 product" item-order="{{sortOrder}}" item-name="{{description.name}}" item-price="{{price}}" data-id="{{id}}">
								<div id = "img-{{description.id}}" class="thumbnail product-img">
                                    {{#image}}
                  <a onclick="addToHistory(event)" data-id="{{id}}" href="<c:url value="/shop/product/" />{{description.friendlyUrl}}.html">
								...
								<div class="product-content text-center">
									<a class="listing-product-name" href="<c:url value="/shop/product/" />{{description.friendlyUrl}}.html"><h3 onclick="addToHistory(event)" data-id="{{id}}" itemprop="name">{{description.name}}</h3></a>
  ...
  <script src="../../../../resources/js/alex.js"></script>
