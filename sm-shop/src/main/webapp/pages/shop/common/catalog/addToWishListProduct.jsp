...
$(function(){
	...
	initProduct('<c:out value="${product.id}"/>','#input-<c:out value="${product.id}" />');//pass form div
								...
								<c:if test="${product.quantityOrderMaximum==-1 || product.quantityOrderMaximum>1 && not product.productVirtual}" >
									...
									<button class="btn addToWishList addToWishListButton btn-buy" type="button" productId="<c:out value="${product.id}" />"><s:message code="button.label.addToWishList" text="Add to WishList"/></button>
								</div>
								</c:if>
								</c:if>
